...
| launch | 作用 |
| --- | --- |
| `camera.launch.py` | 封装 Orbbec 相机启动，供其它 launch include |
...
| 配置文件 | 用途 |
| --- | --- |
| `config/orbbec_camera.yaml` | 感知和抓取，启用彩色图、深度图、点云、有序点云 |
| `config/orbbec_handeye_camera.yaml` | 手眼标定，只启用彩色图和相机内参 |

抓取和感知链路以 `/camera/depth/points` 为点云输入。旧的 `/camera/depth_registered/points` 不作为当前主链路判断依据。
...
- `perception.launch.py` 会覆盖 YOLO 的 `point_cloud_topic` 为 `/camera/depth/points`。
