...
| launch | 作用 |
| --- | --- |
| `camera.launch.py` | 封装 Orbbec Gemini 330 相机启动，供其它 launch include |
...
| 配置文件 | 用途 |
| --- | --- |
| `config/orbbec_gemini_330.yaml` | 感知和抓取，启用彩色图、深度图、对齐点云、有序点云、空洞填补和空间滤波 |
| `config/orbbec_handeye_camera.yaml` | 手眼标定，只启用彩色图和相机内参 |

抓取和感知链路以 `/camera/depth_registered/points` 为点云输入。
...
- `perception.launch.py` 会覆盖 YOLO 的 `point_cloud_topic` 为 `/camera/depth_registered/points`。
