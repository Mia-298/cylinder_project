#!/usr/bin/env python3
"""Manual 11-point, 50-frame object distance test.

Usage:
1. Start the normal perception/robot services.
2. Move the robot manually to a test point.
3. Press Enter. The script records the current TCP pose and 50 fitted object
   positions transformed to the robot base frame.
4. Repeat until 11 points are recorded. The script then exits automatically.
"""
...
def default_output_path() -> Path:
    ...
    return Path.home() / "Desktop" / f"gas_manual_11point_50frame_log_{timestamp}.txt"
...
class ManualLoggerNode(Node):
    def __init__(self, robot_pose_service: str, yolo_service: str, timeout: float) -> None:
        super().__init__("gas_manual_11point_50frame_test")
...
def write_header(file_handle, args: argparse.Namespace, handeye_file: str) -> None:
    file_handle.write("# gas manual 11-point 50-frame object position log\n")
    ...
    file_handle.write(f"# max_points: {args.points}\n")
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        with output.open("w", encoding="utf-8") as file_handle:
            ...
            print()
            print(f"本脚本固定采集 {args.points} 个手动点，每个点采集 {args.samples} 帧。")
            print("每次手动移动到测试点后按 Enter 开始记录；可输入文字作为当前点标签。")
            print("输入 q/quit/exit 可提前结束。")

            for index in range(1, args.points + 1):
                user_input = input(f"\n移动到第 {index}/{args.points} 个点后按 Enter 记录，或输入标签/q: ").strip()
                ...
                print(
                    f"[INFO] point {index}/{args.points} start: "
                ...
                for sample_index in range(1, args.samples + 1):
                    ...
                    if sample_index == 1 or sample_index == args.samples or sample_index % 10 == 0 or not object_valid:
                        print(
                            f"[SAMPLE] point={index}/{args.points} frame={sample_index}/{args.samples} "
                ...
                print(f"[OK] point {index}/{args.points} finished: valid_samples={valid_count}/{args.samples}")
                if index < args.points:
                    print("请手动移动到下一个点。")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manual 11-point, 50-frame logger for TCP pose and fitted object position."
    )
    ...
    parser.add_argument("--samples", type=int, default=DEFAULT_SAMPLES_PER_POINT)
