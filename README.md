...
本仓库是 ROS2 Humble 工作空间，集成奥比中光 Orbbec Gemini 330 相机、YOLOv8 ONNX 检测、有序点云球拟合、Charuco 手眼标定、标定精度验证、AUBO 机器人服务封装和一次抓取执行节点。
...
│   └── third_party/                  # AUBO / ONNX Runtime 等第三方依赖
...
| `src/third_party` | AUBO、ONNX Runtime 等第三方依赖 | `src/third_party/README.md` |
| `third_party` | 旧层级遗留依赖说明 | `third_party/README.md` |

第三方包目录中的 README 属于依赖自身文档，本工程不覆盖。
...
- Ubuntu 22.04
- ROS2 Humble
- Orbbec Gemini 330 相机和 `orbbec_camera` ROS2 包
...
| 模块 | 命令 | 说明 |
| --- | --- | --- |
| 相机 | `ros2 launch gas_bringup camera.launch.py` | 只启动 Orbbec Gemini 330 相机驱动 |
...
这些入口都围绕 Orbbec 的 `/camera/color/image_raw`、`/camera/color/camera_info` 和 `/camera/depth_registered/points`。
...
当前主链路使用 `/camera/depth_registered/points`，该点云由 Orbbec G330 launch 以彩色对齐、有序格式发布。
...
| --- | --- | --- |
| `0` | 成功 | 已到达目标或已发送运动命令 |
| `-2003` | 没有可用三维球心 | `/camera/depth_registered/points`、`ordered_pc`、ROI 深度点、球体遮挡、反光 |
...
- `perception.launch.py` 和 `grasp_pipeline.launch.py` 会把 YOLO 点云话题设置为 `/camera/depth_registered/points`。
